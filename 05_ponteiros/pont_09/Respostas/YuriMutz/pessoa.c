...
void LePessoa(tPessoa *pessoa){
        ...
        //Problema com um \n logo no inicio;
        //Nao consigo limpar o buffer antes de ler a string, da segmentation fault;
...
void AssociaFamiliasGruposPessoas(tPessoa *pessoas){
    ...
    //Nao verifiquei se a logica esta correta
    //                  por causa do problema na LePessoa;

    int familias = 0;
    scanf("%d", &familias);

    // Verifica se leu o familias;
    if(familias != 0){
        printf("oi %d oi\n", familias);
    } else {
        printf("Errado\n");
